...
const StyledParagraph = styled(Typography)<{ $fontColor?: string; $fontFamily?: string }>`
...
const paragraphBlocks: Pick<BlocksStore, 'paragraph'> = {
  paragraph: {
    ...
    handleConvert(editor) {
      baseHandleConvert<Block<'paragraph'>>(editor, { type: 'paragraph' } as Block<'paragraph'>);
    },
    handleEnterKey(editor) {
      ...
      const parentBlockEntry = Editor.above(editor, {
        match: (node) => !Editor.isEditor(node) && 'type' in node && node.type !== 'text',
      });
      if (!parentBlockEntry) {
        return;
      }
