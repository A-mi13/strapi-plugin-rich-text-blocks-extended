...
const Input = React.forwardRef<HTMLInputElement, InputProps>((props, ref) => {
  // Функция для очистки данных rich text полей
  const cleanBlocksData = (data: any): any => {
    // Возвращаем данные как есть, без очистки
    return data;
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    const cleanedValue = cleanBlocksData(value);

    props.onChange({
      target: {
        name,
        type: 'json',
        value: JSON.stringify(cleanedValue),
      },
    });
  };
