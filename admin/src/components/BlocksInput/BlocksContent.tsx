...
interface ExtendedRenderLeafProps extends RenderLeafProps {
  leaf: RenderLeafProps['leaf'] & { className?: string; color?: string };
}
  ...
  const renderLeaf = React.useCallback((props: ExtendedRenderLeafProps) => {
    ...
    return (
      <span 
        ...
        style={{
          color: props.leaf.color || '#FFFFFF',
        }}
      >
        {wrappedChildren}
      </span>
    );
  }, []); // Убираем зависимость от modifiers, чтобы renderLeaf не пересоздавался
