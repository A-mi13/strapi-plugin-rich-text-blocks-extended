...
const StyleToolbar = () => {
  ...
  const handleFontColorChange = (value: string | number) => {
    ...
    // Применяем цвет только к выделенному тексту
    if (Range.isExpanded(editor.selection)) {
      // Если есть выделение, применяем к выделенному тексту
      Editor.withoutNormalizing(editor, () => {
        if (editor.selection) {
          // Применяем цвет к выделенному тексту
          Transforms.setNodes(
            editor,
            { color: stringValue } as any,
            { 
              match: Text.isText,
              split: true,
              at: editor.selection
      ...
      // Если нет выделения, не применяем цвет
      console.log('Нет выделенного текста для применения цвета');
    }
  };
        ...
        {showFontOptions && (
          <ColorPickerWrapper>
            <input
              ...
              title="Выберите цвет для выделенного текста"
            />
          </ColorPickerWrapper>
        )}
